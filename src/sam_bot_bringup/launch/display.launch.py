...
def generate_launch_description():
    ...
    pkg_project_gazebo = get_package_share_directory('sam_bot_gazebo')
    pkg_project_description = get_package_share_directory('sam_bot_description')

    # model path
    default_model_path = os.path.join(pkg_project_description, "urdf/sam_bot_description.urdf"
    )
    ...
    # gz model path
    gz_models_path = os.path.join(pkg_project_gazebo, "models")
    ...
    robot_localization_node = Node(
        ...
        output="screen",
        parameters=[
            os.path.join(pkg_project_bringup, "config/nav2/ekf.yaml"),
